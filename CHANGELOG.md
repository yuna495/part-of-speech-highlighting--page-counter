...
- 文字列一括置換：範囲選択中は、その範囲内のみを置換の対象と見なします。
- オリジナルサイドビューのコンテナタイトル部分に、『新規ファイル作成』と『外部エクスプローラーで表示』アイコンを追加。
- 選択文字列、外部ブラウザでのweblio検索。（`ctrl + t`）
- `ctrl + ← / →`：日本語の文節区切りに合わせてカーソル移動。

### 変更

- マークダウンファイルのコードフェンス内のハイライトを無効。（markmapを除く）
- ルビ内容記入ウインドウの表示について。カーソルが外れる現象を改善するために僅かに遅延を追加。
- プレビュー表示を改変。巻物表示からページ表示へ。
- 拡張機能の軽量化： esbuild によるバンドル導入

### 修正

- kuromoji辞書の重複によるファイルサイズ増大を解決。
...
- サイドバー下側コンテナから、`新規小説を追加`ボタンを削除
  - デフォルトエクスプローラーで、フォルダ右クリックメニューに追加
...
- 見出し行の文字数に、コードフェンスによるコメント部が計上されていた不具合を修正
- ルビ入力ウインドウのフォーカスが外れてしまうことがある問題を改善
- 全角/半角スペースのハイライトを、下線ではなく、設定済み"fwspace"色で背景塗りに変更
- サイドバー見出しの右側に字数デコレーションを表示し、行番号表示を廃止
...
- アクティビティバーのアイコンを変更。
- サイドビューのコンテナタイトル部分を、フォルダ名表示に変更。
- サイドビューツリー（各種操作・ファイル）の最上段に表示されていた「新しい小説の作成」、「親フォルダの .txt を結合」「親フォルダの .md を結合」ボタンを削除。
  - 代わりに**コンテナタイトル部分（ビュータイトル右側）**へボタンを配置しました。
- ルビ挿入方法変更
  - ルビ挿入時に入力ダイアログを追加し、《》内の文字をユーザー入力で自動挿入できるように変更。
...
- サイドビューのツリー表示順優先度をフォルダ→.txt → .md → .json → その他。
...
- **純作業量表示を強化**
  - 実際に入力・削除された文字数を「純作業量」としてステータスバーに表示。
  - クリックで専用のグラフビューを開き、過去30日の推移を可視化。
  - 折れ線と棒グラフを重ねて表示。
  - マウスオーバー時に各日付・詳細値をツールチップ表示。
  - ステータスバーのホバー時には「過去1週間の合計作業量」を表示。
  - グラフ内には赤色の目標ライン（既定 5000字/日）を表示し、設定値によりカスタマイズ可能。
- **語句変換（かな↔漢字）機能を追加**
  - `Ctrl + .` : 「かすか／わずか → 微か／僅か」など、かな表記を漢字表記へ一括変換。
  - `Alt + .` : 「微か／僅か → かすか／わずか」など、漢字表記をかな表記へ一括変換。
- **辞書ファイル**
  ...
  - 辞書ファイルが存在しない場合は既定ペア（かすか/わずか ↔ 微か/僅か）のみ適用。
  - ファイル保存・変更を自動検知し、リアルタイムに再読込されます。
- **サイドバーの機能拡張**
  - サイドビューのコンテナを分割。編集中ファイルと同じ階層の各種ファイルを表示。
  - サイドビュー二つ目のコンテナにおいて、「新しい小説の作成」（雛形作成機能）・「ファイルの結合」を追加。

### 変更

- 全角スペースの下線ハイライトに、半角スペースも適応。
- ステータスバーの文字数表示について、他の拡張子においても適用されるように。

### 修正

- 括弧内のスペースもハイライトされるよう修正。
- 見出しの一括折りたたみ、一括展開トグルを「.md」にも対応するように修正。
