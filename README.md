...
- サイドバーの下側コンテナに、**プロットボードを表示**ボタンを追加。
- **kanbn方式**で、カードの順序を入れ替えてプロット作成に役立ててください。
- カード・列の削除は、右上のゴミ箱へドラッグ＆ドロップで削除されます。※カードは**ファイルごと削除**されるので注意。
- カードをダブルクリックで、タイトル、説明などを直接編集可能。
  - 表示内容は、**タイトル・説明・人物・時間・タグ**。
- 列の名前をダブルクリックで列の名前を変更可能。
- 先頭に固定列 **TBD**（削除・移動不可、暗いグレーで全高表示、出力には含めない）。未定カード置き場にどうぞ。
- タグパレットからカードへドラッグ＆ドロップでタグ付与、カード上のタグを外へドロップで削除。
- タグ色は設定順に表示し、`posNote.kanbn.tagsColors` が1件でもあればデフォルト色を使わない。
- 列色 `posNote.kanbn.columnColors` も1行でも設定されていればデフォルトを使わない。
- board.md に出力（空項目は出力しない、既存ブロックを上書き）。出力すると、末尾に`## 構成`という見出しが追加されます。それ以外の項目はそのまま。
- 列の色設定
...
- カラータグを設定することで、カードの左端がタグに応じて色付けされます。
  - 登録したタグは、上部のタグパレット上に用意。また、カード上で着色されます。
