...
**小説・論文執筆向け**の VSCode 拡張です。（最新版：v3.0.0）
...
- **品詞カラー表示** (v3.0.0にて、kuromoji → wasmに置換)
  - `Rust/WASM` による形態素解析で、名詞 / 動詞 / 形容詞 / 副詞 / 助詞 / 助動詞 / 連体詞 / 接続詞 / 感動詞 / 接頭詞 / その他を色分け
- **括弧・記号ハイライト**
...
- プレビュー下部右側に**文庫サイズでPDF出力**ボタン
  - **注意**：PDF出力には時間が掛かります。
- クリックすることで、プレビュー中の組版を用いてカレントディレクトリにPDFファイルで出力
- フォント設定（デフォルト）
...
- 保存時にテキストリント（文章校正）を実施します。
  ...
  - **特定フォルダ・ファイルのリント除外（`lintignore`）** (v3.0.0)
    ワークスペースの `.vscode/notesetting.json` に `"lintignore"` を定義することで、特定のフォルダやファイルのリント（文章校正）処理を一切実行しないように除外することができます（除外されたファイルはエディタ上の指摘表示も自動でクリアされます）。
    ※追加後は、VSCodeを再起動してください。
    ...
    - **記入法**:
      ...
      - `/A` のようにフォルダを指定すると、そのフォルダ配下にあるすべてのファイル（サブフォルダ内も含む）が除外されます。
      - `/B/C.txt` のようにファイルを指定すると、その特定のファイルのみが除外されます。
      - パスはワークスペースのルートフォルダからの相対パスで記述します（先頭のスラッシュ `/` は省略可能で、Windows環境でも `/` を使用して記述できます）。
...
- 品詞ハイライトが適応されていない間、リント中はファイル切替などをできるだけ避けてください。
