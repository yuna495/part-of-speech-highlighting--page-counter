...
**小説・論文執筆向け**の VSCode 拡張です。（最新版: v2.3.0）
...
## 開発者向けメモ：元ファイルへコメント変更を取り込むには

リポジトリに追加した日本語コメントを別環境の元ファイルへ反映したい場合、以下のいずれかの方法を使うと安全です。

1. **Git コミットを取り込む**  
   - 変更元のリポジトリで `git log` を確認し、目的のコミットハッシュ（例: `e2130a7`）を控えます。  
   - 反映先で `git fetch` 後、任意のブランチにチェックアウトして `git cherry-pick e2130a7` を実行すると、コメント追加だけを移植できます。  
   - コンフリクトが出てもコメント変更なので手動で解消しやすいのが利点です。
2. **パッチとして適用する**  
   - 変更元で `git format-patch -1 e2130a7` などを実行して `.patch` ファイルを作成します。  
   - 反映先で `git apply <作成したパッチ>` を実行すると、同じ差分を再現できます。
3. **ファイルを直接コピーする**  
   - 差分がコメントのみの場合は、対象ファイルを開いてコメント部分をコピー＆ペーストする方法でも問題ありません。  
   - VS Code の「比較」機能を使うと、元ファイルとの違いを目視で確認しながら移植できます。

いずれの方法でも動作ロジックは変わらないため、コメントを取り込むだけなら既存のテストやビルドに影響はありません。
